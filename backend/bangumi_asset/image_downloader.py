...
def download_anime_images(json_file_path: str, output_dir: str):
    ...
    for anime in animes:
        ...
        # 清理文件名中的非法字符
        safe_anime_name = "".join(c for c in anime_name if c.isalnum() or c in (' ', '-', '_')).rstrip()

        if not image_url:
            print(f"番剧 '{safe_anime_name}' (ID: {anime_id}) 没有找到 'large' 类型的图片链接。")
            continue
        ...
        try:
            ...
            print(f"下载番剧 '{safe_anime_name}' (ID: {anime_id}) 的图片时发生网络错误: {e}")
        except Exception as e:
            print(f"处理番剧 '{safe_anime_name}' (ID: {anime_id}) 时发生未知错误: {e}")

if __name__ == '__main__':
    parser = ArgumentParser()
    parser.add_argument("-j", "--json_path", type=str, required=True, help="JSON文件路径")
    parser.add_argument("-o", "--output_folder", type=str, required=True, help="输出文件夹路径")
