...
from backend.bangumi_asset.bangumi_api import get_related_characters, get_character_by_id
...
def enrich_anime_data():
    ...
    # 预先加载已有的角色ID，避免重复请求
    existing_character_ids = {int(p.stem) for p in CHARACTERS_DIR.glob('*.json') if p.stem.isdigit()}
    ...
    for i, anime_data in enumerate(anime_list):
        ...
        # 2. 筛选主角
        main_characters = [char for char in related_characters if char.relation == '主角']
        print(f"  发现 {len(main_characters)} 位主角。")
        
        # 将主角信息存入动画数据
        anime_data['main_characters'] = [char.model_dump(mode="json", by_alias=True) for char in main_characters]
        ...
        for char_summary in main_characters:
            ...
            if char_id in existing_character_ids:
                print(f"    - 主角 '{char_name}' (ID: {char_id}) 的数据已存在，跳过获取。")
                continue
            ...
            try:
                ...
                with open(char_path, 'w', encoding='utf-8') as f:
                    json.dump(character_details.model_dump(mode="json", by_alias=True, exclude_none=True), f, ensure_ascii=False, indent=4)
