...
export type SquadArchetype = 'striker' | 'guardian' | 'support' | 'controller' | 'arcane' | 'tactical';

/** 被动/长效状态的「整场战斗」时长基准（≥ 此值时描述记为「整场战斗」）。 */
export const PASSIVE_DURATION_MS = 90_000;

/** 各定位的通名标签（无个人技名 / 无覆盖时回落到 `${角色名}·${label}`）。 */
export const archetypeLabels: Record<SquadArchetype, {
  skill1: string;
  skill2: string;
  passive: string;
  ultimate: string;
...
/** 各定位的共享技能效果模板（skill1/skill2/passive/ultimate 的 target + effects）。 */
