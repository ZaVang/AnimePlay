...
<template>
  <div class="bg-cream-100 p-6 rounded-lg shadow-lg border border-warm-400 h-full flex flex-col">
    ...
    <div class="border-b border-warm-400 mb-4">
      <nav class="-mb-px flex space-x-4">
        <button @click="activeTab = 'anime'" 
                :class="[activeTab === 'anime' ? 'border-green-400 text-teal-primary' : 'border-transparent text-gray-600 hover:text-white hover:border-warm-300']"
                class="whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm">
          动画 ({{ favoriteAnimeCards.length }}/10)
        </button>
        <button @click="activeTab = 'character'" 
                :class="[activeTab === 'character' ? 'border-green-400 text-teal-primary' : 'border-transparent text-gray-600 hover:text-white hover:border-warm-300']"
                class="whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm">
    ...
    <div class="flex-grow overflow-y-auto max-h-[600px]">
        <div v-if="userStore.isLoggedIn">
            <div v-if="activeTab === 'anime'">
                ...
                <div v-else class="text-center py-8 text-gray-600">
                    <p>还没有喜爱的动画卡牌。</p>
                    <p class="text-sm mt-2">去 <RouterLink to="/collections" class="text-teal-primary hover:underline">收藏</RouterLink> 页面点亮星星吧！</p>
                </div>
            </div>
        ...
        <p v-else class="text-gray-600 text-center py-4">请先登录查看喜爱收藏</p>
    </div>
  </div>
</template>
