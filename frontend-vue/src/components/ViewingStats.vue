...
const progressLevel = computed(() => {
  ...
  if (stats.value.consecutiveDays >= 7) return { level: '爱好者', color: 'text-teal-primary', icon: '⭐' };
  if (stats.value.consecutiveDays >= 3) return { level: '初级', color: 'text-yellow-400', icon: '🌟' };
  return { level: '新手', color: 'text-gray-600', icon: '🌱' };
});
</script>
...
<template>
  <div class="bg-cream-100 p-6 rounded-lg shadow-lg border border-warm-400">
    ...
    <div v-if="userStore.isLoggedIn" class="space-y-6">
      <!-- 观看等级 -->
      <div class="text-center p-4 bg-warm-300/50 rounded-lg">
        ...
        <p class="text-sm text-gray-600">连续观看 {{ stats.consecutiveDays }} 天</p>
      </div>

      <!-- 统计数据网格 -->
      <div class="grid grid-cols-2 gap-4">
        <!-- 总观看时间 -->
        <div class="bg-warm-300/30 p-4 rounded-lg text-center">
          <div class="text-2xl font-bold text-blue-400">
            {{ totalWatchedHours }}h {{ totalWatchedMinutes }}m
          </div>
          <div class="text-xs text-gray-600 mt-1">总观看时间</div>
        </div>

        <!-- 已观看动画数 -->
        <div class="bg-warm-300/30 p-4 rounded-lg text-center">
          <div class="text-2xl font-bold text-teal-primary">{{ userStore.playerState.watchedAnime.size }}</div>
          <div class="text-xs text-gray-600 mt-1">已观看动画</div>
        </div>
      </div>

      <!-- 类型偏好 -->
      <div v-if="topGenres.length > 0">
        <h4 class="text-sm font-medium text-gray-600 mb-3">喜欢的类型</h4>
        <div class="space-y-2">
          <div v-for="[genre, count] in topGenres" :key="genre" class="flex items-center justify-between">
            ...
            <div class="flex items-center">
              <div class="w-12 bg-warm-400 rounded-full h-2 mr-2">
                <div 
      ...
      <!-- 下一个里程碑 -->
      <div class="mt-4 p-3 bg-warm-300/20 rounded-lg border border-warm-300">
