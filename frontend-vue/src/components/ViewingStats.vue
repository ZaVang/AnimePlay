...
<template>
  <div class="bg-surface p-6 rounded-lg shadow-lg border border-line">
    ...
    <div v-if="userStore.isLoggedIn" class="space-y-6">
      <!-- 观看等级 -->
      <div class="text-center p-4 bg-surface-2/50 rounded-lg">
        ...
        <p class="text-sm text-ink-2">连续观看 {{ stats.consecutiveDays }} 天</p>
      </div>
      ...
      <div v-if="topGenres.length > 0">
        <h4 class="text-sm font-medium text-ink-2 mb-3">喜欢的类型</h4>
        <div class="space-y-2">
          <div v-for="[genre, count] in topGenres" :key="genre" class="flex items-center justify-between">
            ...
            <div class="flex items-center">
              ...
              <span class="text-xs text-ink-2 w-6 text-right">{{ count }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- 下一个里程碑 -->
      <div class="mt-4 p-3 bg-surface-2/20 rounded-lg border border-line">
    ...
    <div v-else class="text-center text-ink-2 py-8">
      <svg class="w-12 h-12 mx-auto mb-4 text-ink-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
