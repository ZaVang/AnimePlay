    ...
    <div v-if="authStore.isLoggedIn" class="space-y-4">
      ...
      <div>
        <div class="flex justify-between items-end mb-1">
          <span class="font-bold text-green-400 text-lg">等级 {{ authStore.level }}</span>
          <span class="text-sm text-gray-400">
            {{ authStore.exp }} / {{ authStore.expToNextLevel }} EXP
          </span>
        </div>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4">
        <div class="bg-gray-700 p-4 rounded-lg text-center">
          ...
          <p class="text-2xl font-bold text-white">{{ economyStore.knowledgePoints }}</p>
        </div>
        <div class="bg-gray-700 p-4 rounded-lg text-center">
          <p class="text-gray-400 text-sm">动画抽卡券</p>
          <p class="text-2xl font-bold text-white">{{ economyStore.animeGachaTickets }}</p>
        </div>
        <div class="bg-gray-700 p-4 rounded-lg text-center">
          <p class="text-gray-400 text-sm">角色抽卡券</p>
          <p class="text-2xl font-bold text-white">{{ economyStore.characterGachaTickets }}</p>
        </div>
        <div class="bg-gray-700 p-4 rounded-lg text-center">
          <p class="text-gray-400 text-sm">已存卡组</p>
          <p class="text-2xl font-bold text-white">{{ Object.keys(deckStore.savedDecks).length }}</p>
        </div>
      </div>
