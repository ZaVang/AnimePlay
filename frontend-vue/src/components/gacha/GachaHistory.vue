  ...
  <div>
    ...
    <div v-if="historyWithDetails.length > 0" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="max-h-96 overflow-y-auto pr-2">
        <ul class="space-y-2">
          <li v-for="(item, index) in [...historyWithDetails].reverse()" :key="index" class="flex justify-between items-center p-2 rounded-md bg-surface-2">
            <div class="flex items-center">
              <span 
                class="font-bold px-1.5 py-0.5 rounded text-xs text-white" 
                :class="rarityConfig[item.rarity]?.c.includes('from') ? `bg-gradient-to-r ${rarityConfig[item.rarity]?.c}` : rarityConfig[item.rarity]?.c || 'bg-gray-500'"
              >
                {{ item.rarity }}
              </span>
              <span class="ml-3 font-medium text-sm truncate" :title="item.name">{{ item.name }}</span>
            </div>
            <span class="text-xs text-ink-2 flex-shrink-0 ml-2">{{ formatTime(item.timestamp) }}</span>
          </li>
        </ul>
      </div>
    </div>
