/**
 * 结算表特征测试（S1 建立，S2 随 RewardCalculator 迁入 engine/battle/rewards）。
  ...
  for (const t of tables) {
    describe(t.name, () => {
      it.each(t.rows)('强度差 %i → 攻 %i / 防 %i / 议题 %i', (diff, atk, def, bias) => {
        const r = calculateRewards(clash(diff, t.style));
...
describe('强度差分档边界', () => {
  ...
  it('diff=4 仍是轻微优势（-3），diff=5 进入压倒（-4）', () => {
    expect(calculateRewards(clash(4, style)).defenderReputationChange).toBe(-3);
    expect(calculateRewards(clash(5, style)).defenderReputationChange).toBe(-4);
  });
  it('diff=-4 仍是轻微劣势（攻 -3），diff=-5 进入压倒性劣势（攻 -4）', () => {
    expect(calculateRewards(clash(-4, style)).attackerReputationChange).toBe(-3);
    expect(calculateRewards(clash(-5, style)).attackerReputationChange).toBe(-4);
  });
  it('diff=1 即为轻微优势', () => {
    expect(calculateRewards(clash(1, style)).defenderReputationChange).toBe(-3);
  });
});
...
describe('缺省与兜底', () => {
  it('未选择防御方式时按「赞同」结算', () => {
    const r = calculateRewards(clash(5, { attackStyle: '友好安利' }));
  ...
  it('未传 attackerStrength 时回退用卡面点数', () => {
    ...
    expect(calculateRewards(c).defenderReputationChange).toBe(-4);
  });
});
