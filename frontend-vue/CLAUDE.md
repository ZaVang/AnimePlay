...
**Skill system**: 132 UR skills (66 chars × 2) defined in `data/urCharacterSkillsGenerated.ts` (generated by `scripts/generateUrSkills.js` — do not hand-edit; edit `docs/UR角色技能设计.md` then regenerate；脚本路径已修为仓库相对，本机可跑，产物稳定零 churn). 分发：`runEffect(effectId)` → 真实现 handler（`skills/effects/customHandlers.ts`，48 个）→ 播报表（`engine/skills/announcements.ts`，88 个未实装占位）。S8b/c 真实现一个就从播报表删一个。**S8a 已接通四个消费钩子**：减费入扣费点、禁技入 canUseSkill、强制行动入攻击风格闸口、**被动光环事件管线**（只触发主辩手 + 只分发真实现）；未实装技能 UI 挂「⚠️ 未实装」徽章（`isSkillImplemented`）。新被动写 handler 必须带 event 守卫并与设计描述的触发频率一致，否则进不了管线（参照 S8a 降级教训）。
