...
- ✅ **小队技能 per-character 差异化 + 技能配置表拆分（纯前端）** — 已完成（2026-07-03，见进展）：SSR 开放出战后（2026-07-03，见 [HISTORY.md](HISTORY.md)），可出战池 = **318 人**（UR 67 / HR 110 / SSR 141），但**逐角色技能设计几乎没有**。审计（`squadSkillKits.ts` 全量清点 + 按线上服务名单交叉核算，2026-07-03）实测：真正专属**机制/effect** 差异只有 **10 人（全 UR，`SIGNATURE_KIT_OVERRIDES`）= 3.1%**；另 **80 人**（UR 44 + HR 36）只是**技能名**不同、effect 仍是共享的 6 套 archetype 模板；剩 **228 人（含全部 141 SSR）是名+effect 全通用克隆**。结构性天花板：`getSquadSkillKitForCharacter`（`squadSkillKits.ts:706-751`）五槽中只有 `skill1`/`ultimate` 两槽**能**承载专属 effect，`normalAttack`/`skill2`/`passive` 对任何人恒为模板；且 SSR 约 2/3（battle_stats `hpDef>atkSp` 分支）坍缩到**同一个 guardian 模板**，kit 近乎相同。
  ...
  - **进展（2026-07-03 当日落地）**：① Step1 配置/逻辑拆分 ✅（`a3dce5d`，`data/squad/archetypeTemplates.ts`+`characterKits.ts`）；② Step2 三表并成一张 `CHARACTER_KITS` ✅（`bb1be57`，零行为变更，3647 角色 kit 输出逐一等价）；③ Step3 逐角色设计 **全 318 出战池 ✅**：141 SSR（`44fdeb5`+`b770793`）+ 87 通用 HR/UR（`7135a21`），workflow 生成+人工 balance-lint。**出战池零「名+effect 全通用」克隆**（bespoke 3.1%→约 75%）。④ **设计简化 + 专属被动**（`9a3651a`）：出战 kit 从 5 槽收成 4 槽——移除第二主动技能 skill2、被动升级为逐角色专属 effect；全 238 已 bespoke 角色补专属被动。轮换 = 普攻穿插 → 技能(冷却) → 大招(能量)，被动开场持续整场。**收尾**：① **80 个 name-only 补成三槽全专属 ✅**（`f1ca0e6`）→ 全 318 出战角色 100% 逐角色 bespoke、零模板回落；③ **shield 状态型 buff 修复 ✅**（`fd9ffa2`，改标准 shield effect + validateSlot 守卫）。② **§5 测试/线上数据不一致 修复 ✅**：`squadSkillKits.test.ts` 改读服务同源 `selected_character`，coverage/ready 断言现守真实出战名单。**本项全部收尾完成，无剩余。**
