...
产品进化成熟度（reviewer 评分）6.0 → 7.0 → 7.5。测试 305 → **372**。存档协议 **v7**。**剩余 backlog**（reviewer 提出、未做）：**按声优收集维度**（差异化最强但跨栈——需后端吐 `角色id→声优` 映射，`main_characters[].actors` 被 server.py 服务时剥离）。
