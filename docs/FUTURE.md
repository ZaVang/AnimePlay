...
产品进化成熟度（reviewer 评分）6.0 → 7.0 → 7.5 → 8.0。测试 305 → **400**（后经 2026-06-17 战斗可读性还债两批升至 **418**）。存档协议 **v11**。**✅ 小游戏 5 轮 evolution 全部完成**：统一 🎮 小游戏 Hub 含 **4 个游戏**（猜角色 / 高低牌 / 番剧问答 / 每日挑战）+ 完整留存闭环（每日/周任务 + 成就 + 红点 + 连续 streak）。**剩余**：S11（React 迁移）/ S12（权威后端+多人/PvP）—— FUTURE.md 内决策门控的演进方向，已细拆到 `docs/SPRINT.md` 待 `/think` 立项；backlog：按声优收集维度（跨栈，需后端吐 `角色id→声优` 映射）。

---

### 🔧 战斗可读性还债（2026-06-17，由宅理论战直观度审计驱动）

S8 把技能「真实现」了，但 2026-06-17 的全量复审（132 技能 × 注册表交叉比对）+ 宅理论战直观度审计（4 视角，均 2/5）发现两类残留并已修复：
...
**直观度还债**（核心结论：决定每次对撞胜负的「卡牌强度」全程不显示是最大劝退点；模型结构上比一般战斗难懂，但大半是执行没说清，走「留住模型+重投可读性」路径）：
...
**第二批可读性（2026-06-17 续，已完成）**：① 对撞区强度拆「卡面 + 光环/效果」分解 + 防御阶段即显攻方最终强度（`ClashZone` + 新建 `skills/strengthPreview.ts` 非消费预览 + `status.ts` `hasAuraSuppression`）② 防御弹窗出牌前预判「你 X · 对手 Y · 净差→档位」（`CardActionModal`）③ 结算浮动显示声望/议题增减（`ClashInfo` 加 `rewards?` 类型）。测试 413 → **418**（+预览 helper），preview 实测确认（防御阶段攻方强度可见、预判与结算档位一致、浮字数值吻合）。

**剩余/已知项**：
- ⚠️ **beforeResolve 临时强度不参与结算档位**（gameplay bug，非可读性）：`battleFlow.resolveClash` 把 `addStrengthBonus`（学霸气质/圆环理/运动天赋/SOS团氛围 等 beforeResolve "+N强度" 被动）只加进**展示强度**，rewards 档位用的是不含它的 `baseStrengths`（注释"沿袭原行为"）——等于这些被动**不影响胜负**，只改显示数字。需决策：让其计入档位（增强），还是从展示中剔除（诚实）。属技能"行为=描述"复核的延伸。
...
> **2026-06-17 复审修正**：S8c「技能描述与实际行为一致」当时**未完全达成**——后续全量复审仍查出 9 条描述≠行为残留（3 高危）+ `drawCards` 静默 no-op，已在上方「🔧 战斗可读性还债」节修复对齐。结论：注册全接通属实，但「行为=描述」需逐技能复核，不能只看注册表。
