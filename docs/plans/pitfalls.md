...
## 存档 / 持久化
- 存档协议当前 **v4**。新增字段（如 `saveVersion`）必须**三处同改**：`infra/persistence/schema.ts` + `migrations.ts` + `stores/persistence.ts` 装配器，并补迁移测试（旧档缺省值），**不破坏现有 v4 往返保真测试**（`migrations.test.ts`）。
...
- [并发] saveVersion（保存计数，乐观并发）≠ schema `version`（协议版本，现 5）。后端权威递增、客户端基线 ≠ 服务端当前 → 409；前端 `currentSaveVersion` 随 load/save 更新。
