...
> **实施状态（S8a/b/c 完成后，2026-06-12）**：**全部 66 位 × 2 = 132 条技能已真实现，播报式假实现机制已删除。**
> - 技能定义固化于 `frontend-vue/src/data/urCharacterSkillsGenerated.ts`（由 `scripts/generateUrSkills.js` 生成，请勿手动编辑该文件，改设计请改本文档后重新生成）。
> - **本文描述 = 运行时行为**：S8 重构期间约 20 条技能按可实现机制做了描述微调（已同步本文），审计指出的「说一套做一套」系统性差距已消除。
> - handler 分桶见 `skills/effects/handlers/`（复活被动/内容被动/内容主动/系统技/交互技）+ 条件费用注册表 `costModifiers.ts`；新增技能必须真实现并带特征测试，禁止再造播报式占位。

## 概述

本文档用于设计所有UR稀有度角色的专属技能。系统中 N/R/SR/SSR/HR 级别的角色使用基于稀有度的默认技能模板（见 `data/animeDefaultEffects.ts`），仅 UR 级别角色拥有专属的个性化技能。

共设计 **66 位 UR 角色** 的技能（66 位 × 2 = 132 条全部生成并真实现；S8a 补齐原缺的 #36 志摩凛 / #37 三笠），每个角色包含：
...
- **被动光环**：「隐居创作」- 回合开始时，若己方手牌少于对方，则抽1张牌
...
**技能设计**：
- **主动技能**：「信息操作」(4TP, CD3) - 查看己方牌库顶3张牌，选择1张置于牌库顶
...
- **被动光环**：「轮回记忆」- 己方在对撞中失势（声望受损）时，打出的卡牌有25%几率返回手牌
...
**技能设计**：
- **主动技能**：「节拍调整」(3TP, CD2) - 重置当前主辩手的全部技能冷却
...
**技能设计**：
- **主动技能**：「武士觉醒」(4TP, CD3) - 己方声望≤15时，本回合所有论述+3强度
...
**技能设计**：
- **主动技能**：「爆裂魔法」(6TP, CD5) - 议题偏向巨变+4，但发动后到下回合结束无法打出卡牌
...
**技能设计**：
- **主动技能**：「认真练习」(2TP, CD2) - 本回合及下回合，音乐类卡牌+1强度
...
**技能设计**：
- **主动技能**：「蛇神缠绕」(3TP, CD3) - 对手下一张打出的卡牌强度-2
...
**技能设计**（S8a 补全，刻意只用已实装原语，落地即真实现）：
- **主动技能**：「秘境营地」(3TP, CD3) - 本回合日常类卡牌+2强度，并恢复1点TP
- **被动光环**：「围炉夜话」- 己方打出日常类卡牌时，若手牌≤3张，抽1张牌
...
**技能设计**（S8a 补全，刻意只用已实装原语，落地即真实现）：
- **主动技能**：「立体机动」(3TP, CD3) - 本回合战斗类卡牌+2强度，并抽1张牌
- **被动光环**：「阿克曼血统」- 己方声望≤15时，所有卡牌强度+1
...
**技能设计**：
- **主动技能**：「掌中老虎」(3TP, CD2) - 己方声望≤25（带伤状态）时，本回合所有卡牌+2强度
...
- **被动光环**：「魔法精通」- 己方打出的奇幻类卡牌视为任意类型，可触发类型专精光环
...
**技能设计**：
- **主动技能**：「千花游戏」(2TP, CD1) - 抽2张牌，并标记其中1张：下回合结束前打出时+2强度
...
- **被动光环**：「省力主义」- 回合结束时若本回合未打出任何卡牌，下张卡牌成本-1
...
**技能设计**：
- **主动技能**：「时间警告」(3TP, CD3) - 查看对手1张手牌，并令对手下张打出的卡牌成本+2
- **被动光环**：「未来知识」- 己方打出科幻类卡牌后，下张卡牌+1强度（预判优势）
...
**技能设计**：
- **主动技能**：「AT力场」(3TP, CD3) - 本回合抵消对手对己方的削弱与限制类技能效果，科幻类卡牌+1强度
...
- **被动光环**：「师父照顾」- 打出奇幻类卡牌后，紧接打出的下一张奇幻卡-1费（被其他类型打断则失效）
...
- **被动光环**：「前偶像」- 打出校园类卡牌时本回合恋爱类+1强度；打出恋爱类时本回合校园类+1强度
...
**技能设计**：
- **主动技能**：「不良委员长」(3TP, CD2) - 本回合校园类卡牌+2强度，若以校园类卡牌进行"辛辣点评"则额外+1强度
...
### 67. 蕾塞 (剧场版 链锯人 蕾塞篇)
**角色特征**：炸弹恶魔，伪装天真接近目标的间谍，外表烂漫内里致命
**作品风格**：战斗、恋爱、悬疑

**技能设计**：
- **主动技能**：「爆弹冲击」(3TP, CD3) - 本回合己方战斗类卡牌+3强度，代价己方声望-2
- **被动光环**：「致命诱饵」- 己方打出战斗类卡牌时，对手下一张打出的卡牌强度-1
...
> 本节原为"待新增效果类型"的需求清单。实际实现走了不同的路线，现更新为对照说明。

早期设想的通用效果类型（`VIEW_OPPONENT_HAND_X`、`SWAP_TOPIC_BIAS`、`FORCE_ATTACK_STYLE` 等枚举 ID）**最终未采用**。实际架构改为：

- **每个技能一个中文 `effectId`**（如 `牧濑红莉栖_时间理论`），在 `skills/effects/index.ts` 的 `handlers` 表中注册一个独立闭包，由 `runEffect(effectId, ctx)` 分发。
- **通用能力由两个系统提供**，技能闭包按需调用：
  - `core/systems/InteractionSystem.ts` —— 查看手牌、选牌、选类型等需要玩家交互的能力（对应原设想的"查看手牌类/手牌查看界面"）。
  - `core/systems/PersistentEffectSystem.ts` —— 跨回合的临时加成，如 `addCardTypeStrengthBonus(playerId, '科幻', 2, 1)`（对应原设想的"持续效果类/条件加成类"）。这已是参数化原语的雏形。

**问题在于复用率**：上述正确抽象只有约 3 个技能真正用到，其余 95% 的 handler 是手写的一次性闭包或假实现。技能系统重构的方向应是：**提炼一组参数化原语（加成 / 抽牌 / 查看手牌 / 强制类型…十余个），让 128 个技能从"配置原语"生成，而非各写一个闭包**。

## 待办

1. 补完 #36 志摩凛、#37 三笠·阿克曼 2 位的技能设计
2. 将 112 处 TODO 的"播报式假实现"替换为真实机制（或在 UI 明确标注"未实装"，先消除"说一套做一套"）
3. 技能效果重构为参数化原语，降低复用成本
4. 进行平衡性测试和调整

---

*最后更新：2026-06-12（审计后标注实施状态，对齐实际效果系统架构）*
