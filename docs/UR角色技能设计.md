...
> **实施状态（截至 2026-06）**：本文档列出的技能设计已基本落地。
> - **已生成 64 位 × 2 = 128 条技能定义**，固化于 `frontend-vue/src/data/urCharacterSkillsGenerated.ts`（由 `scripts/generateUrSkills.js` 生成，请勿手动编辑该文件，改设计请改本文档后重新生成）。
> - **仅 2 位仍为空白**：#36 志摩凛、#37 三笠·阿克曼（见下文，技能栏留空）。
> - ⚠️ **数值定义 ≠ 行为实现**：128 条技能的 cost/cooldown/描述是真实的，但 `skills/effects/index.ts` 中约 **78%（112 处带 TODO）的效果 handler 是"播报式假实现"**——UI 会弹出技能生效提示，实际机制并未执行。本文是技能的**设计意图**，与运行时实际行为存在系统性差距（详见 `docs/项目审计报告-2026-06-12.md` 三.3 节）。真正完整实现的范本见牧濑红莉栖「时间理论」。
