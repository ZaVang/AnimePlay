...
**收尾（2026-07-03）**：① **80 个 name-only 补成三槽全专属 ✅**（`f1ca0e6`，44 UR + 36 HR，沿用正统招牌技名）——至此**全 318 出战角色 100% 逐角色 bespoke，零模板回落**（Python 交叉核验 318/318）；③ **shield 状态型 buff 修复 ✅**（`fd9ffa2`：引擎只有 shield *effect* 走 addShield 置吸收池才真减伤，applyStatus 的 shield *状态* 无吸收池被立即滤掉 = 空过；已改标准 shield effect + `validateSlot` 加守卫拒绝 applyStatus{kind:'shield'}）。② **§5 测试/线上数据文件不一致 修复 ✅**：`squadSkillKits.test.ts:26` 改读服务同源的 `data/selected_character/all_cards.json`（2021 人）——coverage/ready 断言现守真实出战名单；顺带移除测试中不在服务名单的 id（利威尔 19546）。**至此本审计三项收尾全部完成。**
